...
float readMagnet_B_total_filtered() {
  ...
  static float emaX = 0.0f, emaY = 0.0f, emaZ = 0.0f;
  static float lastMagnitude = 1e-6f;   // nie bei 0 starten
  ...
  if (now - lastSampleTime < SAMPLE_PERIOD_MS) {
    return lastMagnitude;                 // kein neues Sample → alten Wert
  }
  lastSampleTime = now;
  ...
  if (Tlv493dMagnetic3DSensor.getMagneticFieldAndTemperature(&x, &y, &z, nullptr)) {
    ...
    if (lastMagnitude < 1e-6f) lastMagnitude = 1e-6f;   // clamp
  }
  // Bei Read-Fehler behalten wir lastMagnitude (kein 0!)
...
float B_to_D(float B){
  const float EPS = 1e-6f;         // sinnvoller Mindestbetrag für Setup
  float x = (B < EPS) ? EPS : B;
  return a * logf(x) + b;          // logf für float-Konsistenz
}
...
void showError(const char* msg){
  ...
  const unsigned long now = millis();
  if (now >= nextFlash) {                   // max 5 Hz „Fehlerblinker“
    Serial.println(msg);
    display.clearDisplay();
    display.setCursor(0,0);
    display.setTextSize(1);
    ...
    display.println(msg);
    display.display();
    nextFlash = now + 200;                  // 200 ms, keine delay()
  }
}
...
void requestData() {
  int n = Wire.requestFrom(8, (uint8_t)4);
  if (n != 4) {
    Serial.print("I2C: erwartet 4 Bytes, erhalten=");
    Serial.println(n);
    return;
  }
  ...
  current_rpm = speed; isrunning = running; mode = modeVal;
}
...
void modusPWM() {
  ...
  static float lastValidD = NAN;          // für stabile Anzeige

  float B = readMagnet_B_total_filtered();
  bool measOk = (B >= 1e-6f) && isfinite(B);
  ...
  float D_meas = NAN;
  if (measOk) {
    D_meas = B_to_D(B);                   // intern geclamped
    if (isfinite(D_meas)) lastValidD = D_meas;
  }

  // Display unabhängig vom Mess-Erfolg mit 2 Hz updaten
  if ((int32_t)(millis() - nextDisplay) >= 0) {
    ...
    display.setCursor(0, 36);
    if (isfinite(D_meas)) {
      display.print(D_meas, 2);
      display.println(" mm");
    } else if (isfinite(lastValidD)) {
      display.print(lastValidD, 2);
      display.println(" mm");
    } else {
      display.println(" - ");
    }

    // kleiner „Heartbeat“, damit du siehst, dass die Loop lebt
    display.setCursor(110, 0);
    display.print((millis()/500)%2 ? "." : " ");

    display.display();
    nextDisplay = millis() + 500;        // 2 Hz
  }

  // WICHTIG: kein delay() hier. Alles non-blocking.
}
